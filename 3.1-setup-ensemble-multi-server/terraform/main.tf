...
resource "null_resource" "ProvisionRemoteHostsIpToAnsibleHosts" {
  ...
  provisioner "local-exec" {    
    command = "echo ${element(aws_instance.DevInstanceAWS.*.public_ip, count.index)} myid=${count.index+1} dns=${element(aws_instance.DevInstanceAWS.*.private_dns, count.index)} >> ../hosts"
  }
}
